...
public class Koan3 {
	...
	public void shouldRetrieveCompanionsIndexFromTheDatabase() {
		...
		assertThat(
				companions,
				contains("Rose Tyler", "Adam Mitchell", "Jack Harkness",
	...
	public void addingToAnIndexShouldBeHandledAsAMutatingOperation() {
		...
		try {
			db.index().forNodes("characters")
					.add(nixon, "name", nixon.getProperty("name"));
			tx.success();
		} finally {
			tx.finish();
		}
		...
		assertNotNull(db.index().forNodes("characters")
				.get("name", "Richard Nixon").getSingle());
	}

	@Test
	public void shouldFindSpeciesBeginningWithTheLetterSAndEndingWithTheLetterNUsingLuceneQuery()
			throws Exception {
		IndexHits<Node> species = null;

		// SNIPPET_START

		species = universe.getDatabase().index().forNodes("species")
				.query("species", "S*n");
	...
	@Test
	public void shouldKeepDatabaseAndIndexInSyncWhenCyberleaderIsDeleted()
			throws Exception {
		GraphDatabaseService db = universe.getDatabase();
		...
		assertNull("Cyberleader has not been deleted from the enemies index.",
				enemies.get("name", "Cyberleader").getSingle());
